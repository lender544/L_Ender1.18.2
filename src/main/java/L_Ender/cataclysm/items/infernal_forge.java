...
public class infernal_forge extends PickaxeItem {
    ...
    private void EarthQuake(UseOnContext context) {
        ...
        for (Entity entity : list) {
            if (entity instanceof LivingEntity) {
                entity.hurt(DamageSource.playerAttack(player), (float) player.getAttributeValue(Attributes.ATTACK_DAMAGE));
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
        tooltip.add(new TranslatableComponent("item.cataclysm.infernal_forge.desc").withStyle(ChatFormatting.DARK_GREEN));
        tooltip.add(new TranslatableComponent("item.cataclysm.infernal_forge.desc2").withStyle(ChatFormatting.DARK_GREEN));
    }
}
