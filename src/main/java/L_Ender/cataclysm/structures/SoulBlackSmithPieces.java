...
public class SoulBlackSmithPieces {
    ...
    public static void start(StructureManager templateManager, BlockPos pos, Rotation rotation, StructurePieceAccessor pieceList, Random random) {
        ...
        pieceList.addPiece(new SoulBlackSmithPieces.Piece(templateManager, UPPER_SIDE, blockpos, rotation));

        rotationOffSet = new BlockPos(0, 0, -34).rotate(rotation);
        blockpos = rotationOffSet.offset(x, pos.getY(), z);
        pieceList.addPiece(new SoulBlackSmithPieces.Piece(templateManager, BOTTOM_SIDE, blockpos, rotation));
    }
    ...
    public static class Piece extends TemplateStructurePiece {
        ...
        public Piece(StructureManager templateManagerIn, ResourceLocation resourceLocationIn, BlockPos pos, Rotation rotation) {
            super(ModStructures.SBSP, 0, templateManagerIn, resourceLocationIn, resourceLocationIn.toString(), makeSettings(rotation), makePosition(resourceLocationIn, pos));
        }

        public Piece(StructureManager templateManagerIn, CompoundTag tagCompound) {
            super(ModStructures.SBSP, tagCompound, templateManagerIn, (p_162451_) -> {
